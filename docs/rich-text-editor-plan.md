...
* [ ] **Markdown Paste Interpretation:** Optionally intercept standard copied Markdown plain text and render the equivalent HTML (`**bold**` -> `<b>`, etc.) before dropping it into `contenteditable`.
* [ ] **Markdown Copy (Optional):** Offer a lightweight parse out when the user copies NUI content as plain text, injecting Markdown into their system clipboard instead of just stripped string text.
